...
Material.setup = function () {
  ...
  function generateForce(body) {
    Matter.Body.applyForce(
      body,
      { x: body.position.x, y: body.position.y },
      { x: 0, y: -0.3 }
    )
    Matter.Body.setAngularVelocity(body, Math.PI / 6)
  }
  ...
  class Taskball {
    constructor(w, h, id) {
      ...
      this.body = this.createBody(x, y, id)
    }

    createBody(x, y, id) {
      return Matter.Bodies.rectangle(x, y, this.radius, this.radius, {
        // density: 0.0005,
        // frictionAir: 0.06,
        // restitution: 0.3,
        // friction: 0.01,
  ...
  for (var i = 0; i < numCircle; i++) {
    taskballs.push(new Taskball(w, h, i))
  }
  const balls = taskballs.map((taskball) => taskball.body)
  const canvas = document.querySelector('#task-area')
  ...
  const wall1 = Matter.Bodies.rectangle(-30, h / 2, 60, h * 2, { isStatic: true })
  const wall2 = Matter.Bodies.rectangle(w + 30, h / 2, 60, h * 2, {
    isStatic: true,
  })
  ...
  Matter.Events.on(mouseConstraint, 'mouseup', (event) => {
    ...
    if (body !== undefined) {
      generateForce(body)
      Animation.explode(body.position.x, body.position.y)
    }
  })
